...
_start:
    // this is the switch portion of the case statement
    ...
    cmp x0, #2 // make sure we have precisely two arguements
    bne endit // if not, exit
    ldr x11, [x1, #8] // get the pointer at x1 + 8
    ldrb w11, [x11] // load the byte pointed to by that pointer into w11
    sub w11, w11, #'0' // substract the ascii value of '0' from w11 to get the integer value
...
break:		
    nop	        // nop here just to prove we actually 
